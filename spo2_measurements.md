...
Oxygen and Glucose enable aerobic glycolysis which produces ATP. Oxygen is transported to the cells through perfusion and diffusion. Its taken up from the inhaled air and bound to hemoglobin in red blood cells. This oxygenation is usually efficient and almost all hemoglobin molecules pick up 4 oxygens in the lung's alveoli.
...
Absorption of hemoglobin in the blue would be much stronger, however optical penetration is also affected by increased scattering. To maintain a strong signal on the sensor, measurements occur in the wavelength range of the "optical window" of tissue which is in the red to near infrared where absorption and scattering is reduced.

To simply detect the heart rate, measurements in the green create stronger signal fluctuation as compared to the red or near infrared, however the largest relative absorption differences between oxy and deoxy hemoglobin occur in the red.

**Molar attenunation coeficient** $a(\lambda)$ with units [ $\frac{1}{cm M}$ ] expresses the attenuation properties as function of concentration and pathlength where $M = moles/Liter$. Relevant examples at two wavelengths are given below:
...
Absorbance is expressed as $A = a \cdot c \cdot L$ with $c$ the concentration and $L$ the optical pathlength.
...
with $a$ the optical property and $c$ the concentration.
...
$HbO_2 Sat = \frac{\frac{A_{\lambda_1}}{A_{\lambda_2}} \cdot a_{Hb,\lambda_2} - a_{Hb,\lambda_1} } { \frac{A_{\lambda_1}}{A_{\lambda_2}} \left[ a_{Hb,\lambda_2} - a_{HbO_2,\lambda_2}\right]  + \left[ a_{HbO_2,\lambda_1} - a_{Hb,\lambda_1}\right] }$
...
Therefore the extraction of $PPG_{relaxed}$, $PPG_{extended}$ from two measurement traces at two wavelengths ($R$) as well as the optical constants of oxyhemoglobin and deoxyhemoglobin ($a_{Hb}$, $a_{HbO_2}$) at those wavelengths can be used to solve for

$HbO_2 Sat=\frac{a_{Hb_{660}} - R \cdot a_{Hb_{940}}}{R \cdot (a_{HbO_{2_{940}}}-a_{Hb_{940}}) - (a_{HbO_{2_{660}}}-a_{Hb_{660}})}$
...
The above formulas show that pathlength for the two wavelengths is assumed to be the same, however this is not correct when other optical components become more relevant such as Melanin. 

Typically, fluctuations of the NIR signal are weaker than the ones in the red and the sensor should be operated so that maximum values are measured at both wavelengths by adjusting the LED current when switching between the LEDs. The incident optical power does not need to be the same at both wavelengths as we are creating intensity ratios between the extended and relaxed blood vessel condition and those scale linearly with the incident power. An increased signal will however improve the signal to noise ratio and improve the algorithm to estimate the maximum and minimum of the signal.  
