...
The image on the right represents a Nyquist plot. If the measured circuit would consist of a resistor parallel to a capacitor, the 0Hz value would start at horizontal axis with maximum resistance and with increasing frequency the plot would be follow a half circle and infinite frequency would be again at the horizontal axis but with smallest value.
...
At a given frequency the unknown tissue impedance is given as magnitude $∣Z∣$ and phase $\theta$.

When measurements are conducted at several phase offsets $\phi_i$ we can determine the impedance with a least squares fit. We will record the following values:
